    ...
    match line.trim() {
        ...
        "/tools" => {
            let builtins: Vec<String> = crate::tools::tool_definitions()
                ...
                .collect();
            let mut msg = format!("Built-in tools ({}):\n{}", builtins.len(), builtins.join("\n"));
        ...
        "/config" => {
            ...
            let key_status = if info.api_key_set { "configured" } else { "not set" };
            app.chat_lines.push(crate::tui::app::ChatLine::System(
                format!("Config:\n  Model: {}\n  Base URL: {}\n  API key: {}\n  Max turns: {} (soft: {})\n  Tools: {} ({} MCP)\n  Skills: {}",
        ...
        "/prompt" => {
            let prompt_text = app.agent.messages.first()
            ...
            match crate::session::Session::list_all() {
                Ok(sessions) => {
                    if sessions.is_empty() {
                        app.chat_lines.push(crate::tui::app::ChatLine::System("No sessions found.".into()));
                    } else {
                        let lines: Vec<String> = sessions.iter()
                            .map(|s| format!("  {}... — {} msgs", &s.id[..8.min(s.id.len())], s.message_count))
                            .collect();
                        app.chat_lines.push(crate::tui::app::ChatLine::System(
                            format!("Sessions:\n{}", lines.join("\n")),
                        ));
        ...
        s if s.starts_with("/prompt diff") => {
            let layers = crate::prompt::build_prompt_layers(
                &app.agent.soul, &app.agent.memory, &app.agent.skills, &app.agent.config, &app.agent.resolved.model,
            );
            if let Some(prev) = &app.agent.previous_prompt_layers {
                ...
                let names: Vec<String> = layers.layers.iter().map(|l| format!("  ● {}", l.name)).collect();
                app.chat_lines.push(crate::tui::app::ChatLine::System(
                    format!("Prompt layers (no previous to diff):\n{}", names.join("\n")),
                ));
            }
        ...
        "/undo" => {
            if app.agent.undo() {
                app.chat_lines.push(crate::tui::app::ChatLine::System("Undid last exchange.".into()));
            } else {
                app.chat_lines.push(crate::tui::app::ChatLine::Error("Nothing to undo".into()));
            }
            CommandResult::Done
        }
        other => {
